...
/*  Za  Zb  Zc   theta0(rad)         k(eV/rad²)   description */
    { 1,  8,  1,  DEG2RAD(104.52), 0.834 }, /* H-O-H water      */
    { 1,  7,  1,  DEG2RAD(106.67), 0.630 }, /* H-N-H            */
    { 1,  6,  1,  DEG2RAD(109.47), 0.600 }, /* H-C-H sp3        */
    { 1,  6,  6,  DEG2RAD(109.47), 0.630 }, /* H-C-C            */
    { 1,  6,  7,  DEG2RAD(109.47), 0.630 }, /* H-C-N            */
    { 1,  6,  8,  DEG2RAD(109.47), 0.630 }, /* H-C-O            */
    { 1,  7,  6,  DEG2RAD(118.00), 0.630 }, /* H-N-C            */
    { 1,  8,  6,  DEG2RAD(108.50), 0.814 }, /* H-O-C            */
    { 6,  6,  6,  DEG2RAD(109.47), 0.750 }, /* C-C-C sp3        */
    { 6,  6,  7,  DEG2RAD(109.47), 0.750 }, /* C-C-N            */
    { 6,  6,  8,  DEG2RAD(109.47), 0.750 }, /* C-C-O            */
    { 6,  6, 16,  DEG2RAD(114.00), 0.650 }, /* C-C-S            */
    { 7,  6,  7,  DEG2RAD(116.00), 0.630 }, /* N-C-N            */
    { 7,  6,  8,  DEG2RAD(115.00), 0.700 }, /* N-C-O            */
    { 8,  6,  8,  DEG2RAD(123.00), 0.750 }, /* O-C-O            */
    { 6,  7,  6,  DEG2RAD(111.00), 0.750 }, /* C-N-C            */
    { 6,  8,  6,  DEG2RAD(111.55), 0.814 }, /* C-O-C ether      */
    { 6, 16,  6,  DEG2RAD(102.60), 0.750 }, /* C-S-C            */
    { 6,  6,  6,  DEG2RAD(120.00), 1.000 }, /* C-C=C  sp2 (dup handled by order lookup) */
};
...
PairEnergy forces_nonbonded_pair(Atom *atoms, int ia, int ib,
    ...
    if (use_lj) {
        ...
        /* f_lj = 24ε/r² × (sr6 - 2*sr12)
         * Note sign: positive f_lj means repulsive (pushes apart),
         * so F_i = f_lj × r_ij pushes atom i away from atom j.
         * At small r: sr12 >> sr6, bracket negative → f_lj negative →
         * F_i points opposite to r_ij → repulsion. ✓  */
    ...
    if (use_coulomb) {
        ...
        if (fabs(qi) > 1.0e-9 && fabs(qj) > 1.0e-9) {
            /* V_C = COULOMB_MD × qi × qj / r  [eV] */
            result.coulomb_energy = COULOMB_MD * qi * qj / r;

            /* f_c = -COULOMB_MD × qi × qj / r³
             * F_i = f_c × r_ij:
             *   opposite charges → f_c < 0 → F_i opposite to r_ij → attraction ✓
             *   same charges     → f_c > 0 → F_i along  r_ij  → repulsion  ✓  */
            double f_c = -COULOMB_MD * qi * qj / (r2 * r);
            f_total += f_c;
        }
    }
