...
static int place_molecule(Simulation *sim, Vec3 origin,
    ...
    for (int i = 0; i < n_atoms; i++) {
        ...
        sim_add_atom(sim, Zs[i], p, 0.0);  /* partial charges deferred */
    }
...
int sim_place_uracil(Simulation *sim, Vec3 origin) {
    ...
    return place_molecule(sim, origin, coords, Zs, 12, bonds, 12);
}

/* ══════════════════════════════════════════════════════════════════════════
 * CYTOSINE  (DNA/RNA pyrimidine base)
 * Atom order: N3 C4 N1 C2 O2 N4 C5 C6 HN3 HN41 HN42 H5 H6
 * ══════════════════════════════════════════════════════════════════════════ */
int sim_place_cytosine(Simulation *sim, Vec3 origin) {
    ...
    static const int bonds[13][3] = {
        {0,1,1}, {0,3,1}, {0,8,1},   /* N3-C4, N3-C2, N3-HN3   */
        {1,5,1}, {1,6,2},            /* C4-N4, C4=C5           */
        {2,3,1}, {2,7,2},            /* N1-C2, N1=C6           */
        {3,4,2},                    /* C2=O2                  */
        {5,9,1}, {5,10,1},           /* N4-HN41, N4-HN42       */
        {6,7,1}, {6,11,1},           /* C5-C6, C5-H5           */
        {7,12,1}                    /* C6-H6                  */
    };

    return place_molecule(sim, origin, coords, Zs, 13, bonds, 13);
}
...
int sim_place_adenine(Simulation *sim, Vec3 origin) {
    ...
    return place_molecule(sim, origin, coords, Zs, 15, bonds, 16);
}
...
int sim_place_guanine(Simulation *sim, Vec3 origin) {
    ...
    return place_molecule(sim, origin, coords, Zs, 16, bonds, 17);
}
...
int sim_place_thymine(Simulation *sim, Vec3 origin) {
    ...
    return place_molecule(sim, origin, coords, Zs, 15, bonds, 15);
}
